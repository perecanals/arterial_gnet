...
def main(model_dir, base_model_name, root_test, dataset_name, device=0):
    ...
    # Get data loaders
    test_loader = get_test_data_loader(root_test, base_model_name, None, test_transform, radius=args_.radius, use_lap_pos_enc=False, pos_enc_dim=8)

    model_name = os.path.basename(model_dir)

    if "_cl-" in model_name:
        combined_loss = model_name.split("_cl-")[1].split("_")[0] == 'combined' and base_model_name == "ArterialGNet"
    else:
        combined_loss = False
...
if __name__ == "__main__":
    ...
    base_model_name = "ArterialGNet"

    import time

    start = time.time()

    main(model_dir, base_model_name, root_test, dataset_name)
