...
class HGPSLPool(torch.nn.Module):
    def __init__(self, in_channels, ratio=0.8, sample=False, sparse=False, sl=True, lamb=1.0, negative_slop=0.2):
        ...
        self.pool = TopKPooling(in_channels=self.in_channels, ratio=self.ratio)
    ...
    def forward(self, x, edge_index, edge_attr, batch=None):
        ...
        # Graph Pooling using TopKPooling
        x, edge_index, edge_attr, batch, perm, score = self.pool(
            x=x,
            edge_index=edge_index,
            edge_attr=edge_attr,
            batch=batch,
            attn=score.unsqueeze(-1)
        )
        ...
        if self.sample:
            ...
            new_edge_index, new_edge_attr = add_remaining_self_loops(hop_edge_index, hop_edge_attr, 0, x.size(0))
            ...
            if edge_attr is None:
                edge_attr = torch.ones((edge_index.size(1),), dtype=x.dtype, device=edge_index.device)
            ...
            adj[edge_index[0], edge_index[1]] += edge_attr * self.lamb
            weights = adj[row, col]
...
class HGPSLModel(torch.nn.Module):
    def __init__(self, node_in_dim, hidden_dim, num_classes, pooling_ratio, dropout, sample_neighbor, sparse_attention=True, structure_learning=True, lamb=1.0):
        ...
        self.conv1 = GCNConv(self.node_in_dim, self.hidden_dim)
        self.conv2 = GCN(self.hidden_dim, self.hidden_dim)
        self.conv3 = GCN(self.hidden_dim, self.hidden_dim)
