...
from arterial_gnet.dataloading.utils import load_pickle, z_score_normalization, min_max_normalization, laplacian_positional_encoding, sinusoidal_positional_encoding
...
class ArterialMapsDataset(InMemoryDataset):
    ...
    """
    def __init__(self, root, raw_file_names_list = None, processed_file_names_list = None, pre_transform = None, transform = None, radius = 0, use_lap_pos_enc = False, pos_enc_dim = 8):
        ...
        for raw_file_path in self.raw_file_names:
            ...
            if os.path.exists(preprocessed_file_path):
                ...
                if self.use_lap_pos_enc:
                    dense_data.lap_pos_enc = laplacian_positional_encoding(dense_data.edge_index, dense_data.num_nodes, self.pos_enc_dim)
...
class DenseGraphDataset(InMemoryDataset):
    """
    Dataset class for the Arterial Maps dataset, containing a set of objects encoding
    information for the arterial pathway (supersegment) for a stroke patient who underwent
    endovascular treatment at different scales.
    ...
    def __init__(self, root, raw_file_names_list = None, processed_file_names_list = None, pre_transform = None, transform = None, radius = 0, use_lap_pos_enc = False, pos_enc_dim = 8):
        ...
        self.raw_file_names_list = raw_file_names_list
        self.processed_file_names_list = processed_file_names_list
        self.processed_dir_radius = os.path.join(root, f"processed_only_dense_radius-{radius}")
        self.processed = self.processed_dir_radius
        self.use_lap_pos_enc = use_lap_pos_enc
        self.pos_enc_dim = pos_enc_dim
        if use_lap_pos_enc:
            self.processed_dir_radius += f"_lap_pos_enc-{self.pos_enc_dim}"
        else:
            self.processed_dir_radius += f"_sin_pos_enc-{self.pos_enc_dim}"
        os.makedirs(os.path.join(root, f"processed"), exist_ok=True) # Auxiliary folder for non-radius-based processing (necessary for the code to work)
        os.makedirs(self.processed_dir_radius, exist_ok=True) # Actual folder for radius-based processing
        super(DenseGraphDataset, self).__init__(root=root, pre_transform=pre_transform, transform=transform)
        self.process()
    ...
    def process(self): 
        ...
        for raw_file_path in self.raw_file_names:
            ...
            if os.path.exists(preprocessed_file_path):
                ...
                for src, dst in raw_dense_graph.edges:
                    dense_edge_index.append(np.array([src, dst]))
                data.pos = torch.tensor(np.array(dense_pos), dtype=torch.float32)
                data.x = self.normalize_dense_node_features(torch.tensor(np.array(dense_x), dtype=torch.float32))
                data.edge_index = torch.transpose(torch.tensor(np.array(dense_edge_index), dtype=torch.int64), 1, 0)
                data.num_nodes = len(raw_dense_graph.nodes)
                data.num_edges = len(raw_dense_graph.edges)
                if self.use_lap_pos_enc:
                    data.pos_enc = laplacian_positional_encoding(data.edge_index, data.num_nodes, self.pos_enc_dim)
                else:
                    data.pos_enc = sinusoidal_positional_encoding(data.num_nodes, self.pos_enc_dim)
                data.id = raw_file_path.split(".")[0]
...
class DenseGraphDataset(InMemoryDataset):
    ...
    Dataset class for the Arterial Maps dataset, containing a set of objects encoding
    information for the arterial pathway (supersegment) for a stroke patient who underwent
    endovascular treatment at different scales. This dataset was created for using the graphs
    as normal PyG Data objects for faster processing. Global and segment graphs were not used.
...
class SequenceGraphDataset(InMemoryDataset):
    """
    Dataset class for the Arterial Maps dataset, containing a set of objects encoding
    information for the arterial pathway (supersegment) for a stroke patient who underwent
    endovascular treatment at different scales. This class treats graphs as sequences of nodes 
    with maximum length 256.
    ...
    """
    def __init__(self, root, raw_file_names_list = None, processed_file_names_list = None, pre_transform = None, transform = None, pos_enc_dim = 8, max_seq_len = 256):
        if os.path.exists(os.path.join(root, "dataset.json")):
            pass
        else:
            print("No dataset description file found. Creating dataset description file from raw dir data.")
        ...
        self.raw_file_names_list = raw_file_names_list
        self.processed_file_names_list = processed_file_names_list
        self.processed_dir_seq = os.path.join(root, "processed_only_seq")
        self.processed = self.processed_dir_seq
        self.pos_enc_dim = pos_enc_dim
        self.max_seq_len = max_seq_len
        self.processed_dir_seq += f"_sin_pos_enc-{self.pos_enc_dim}_max_seq_len-{self.max_seq_len}"
        os.makedirs(os.path.join(root, f"processed"), exist_ok=True) # Auxiliary folder for non-radius-based processing (necessary for the code to work)
        os.makedirs(self.processed_dir_seq, exist_ok=True) # Actual folder for radius-based processing
        super(SequenceGraphDataset, self).__init__(root=root, pre_transform=pre_transform, transform=transform)
        self.process()
    ...
    def processed_file_names(self):
        if self.processed_file_names_list is not None:
            ...
            return sorted([f for f in sorted(os.listdir(self.processed_dir_seq)) if f.endswith(".pt")])
        
    def process(self): 
        # Here you would read your raw files, create Data objects, and apply any pre-transforms
        data_list = []
        for raw_file_path in self.raw_file_names:
            preprocessed_file_path = os.path.join(self.processed_dir_seq, "{}.pt".format(raw_file_path.split(".")[0]))
            if os.path.exists(preprocessed_file_path):
                data_list.append(torch.load(preprocessed_file_path))         
            else:
                # Load the raw file
                ...
                data.seq_len = len(data.x)
                if data.seq_len > self.max_seq_len:
                    print(f"Warning: {raw_file_path.split('.')[0]} Sequence length {data.seq_len} is greater than max_seq_len {self.max_seq_len}. Truncating sequence.")
                # Pad x
                data.x = F.pad(data.x, (0, 0, 0, self.max_seq_len - data.seq_len))
                data.mask = torch.arange(self.max_seq_len, device=data.x.device)[None, :] < data.seq_len
                data.pos_enc = sinusoidal_positional_encoding(self.max_seq_len, self.pos_enc_dim)
